...
void delay_1ms (void){
  int i;
  for (i = 0; i < 4; i++) delay_250us();
}

void delay_2ms (void){
  int i;
  for (i = 0; i < 8; i++) delay_250us();
}

void delay_1s (void){
  int i;
  for (i=0;i<4000;i++) delay_250us();
}
	
unsigned int calcul_t_remenador (){
	return (8*PORTCbits.RC1 + 4*PORTCbits.RC0 + 2*PORTCbits.RC3 + 1*PORTCbits.RC2 + 3);
}
	
unsigned int def_estat(){
	unsigned int valor = 8*PORTCbits.RC5 + 4*PORTCbits.RC4 + 2*PORTCbits.RC7 + 1*PORTCbits.RC6;
	if (valor > 0 && valor < 4) return (valor);
	else return (0);
}

unsigned int calc_pos_switch_temps() {
	return (8*PORTCbits.RC1 + 4*PORTCbits.RC0 + 2*PORTCbits.RC3 + 1*PORTCbits.RC2);
}

unsigned int calc_pos_switch_numero() {
	return (8*PORTCbits.RC5 + 4*PORTCbits.RC4 + 2*PORTCbits.RC7 + 1*PORTCbits.RC6);
}
